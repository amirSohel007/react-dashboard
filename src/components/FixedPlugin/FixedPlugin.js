...
import { Badge, Button, Card, Navbar, Nav, Container, Col, Dropdown, Form } from 'react-bootstrap';
					...
					<Dropdown.Menu>
						...
						<li className='adjustments-line d-flex align-items-center'>
							...
							<Form.Check
								...
								checked={imageSwitch}
								onChange={() => {
									imageSwitch ? setSidebarImageParent('') : setSidebarImageParent(sidebarImage);
						...
						<li className='adjustments-line d-flex align-items-center'>
							...
							<Form.Check
								...
								onChange={() => {
									...
									document.querySelector('.main-panel').classList.toggle('mt-5');
								}}
							/>
						</li>
						<li className='adjustments-line d-flex align-items-center'>
							...
							<div>
								{backgroundColors.map((prop, key) => (
									<Badge
										...
										className={prop === backgroundColor ? 'active' : ''}></Badge>
								))}
							</div>
						</li>
						<li className='header-title'>Sidebar Images</li>
						{sidebarImages.map((prop, key) => (
							<li key={key} className={prop === sidebarDefaultImage ? 'active' : ''}>
								<a className='img-holder' onClick={() => setSidebarImageParent(prop)}>
